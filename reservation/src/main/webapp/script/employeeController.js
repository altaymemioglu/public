...
angular.module('hotel').controller('employeeCtrl', ['$scope', '$http', function($scope, $http){
	...
	$scope.save = function () {
		...
		$http({
			method : 'POST',
			url : 'http://localhost:8080/reservation/rest/services/employee/saveemployee',
			data : $scope.employee,
		    headers : {
	...
	$scope.remove = function (emp) {
		...
		$http({
			method : 'POST',
			url : 'http://localhost:8080/reservation/rest/services/employee/removeemployee',
			data : emp,
		    headers : {
		        'Content-Type' : 'application/json',
		        'X-CSRF-TOKEN' : token
	...
	$scope.activities = function (emp) {
		$http({
			method : 'GET',
			url : 'http://localhost:8080/reservation/rest/services/employee/getemployeeactivities?employeeid='+emp.id
		}).then(function successCallback(response) {
