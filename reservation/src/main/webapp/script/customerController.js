...
angular.module('hotel').controller('customerCtrl', ['$scope', '$http', function($scope, $http){
	$scope.fillCustomerList = function () {
		$http({
			method : 'GET',
			url : 'http://localhost:8080/reservation/customers'
	...
	$scope.save = function () {
		...
		$http({
			method : 'POST',
			url : 'http://localhost:8080/reservation/rest/services/customer/savecustomer',
			data : $scope.customer,
		    headers : {
	...
	$scope.remove = function (cust) {
		...
		$http({
			method : 'POST',
			url : 'http://localhost:8080/reservation/rest/services/customer/removecustomer',
			data : cust,
		    headers : {
		        'Content-Type' : 'application/json',
		        'X-CSRF-TOKEN' : token
